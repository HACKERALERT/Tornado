...
class MainWindow(QMainWindow):
	...
	def __init__(self):
		...
		self.needRepaint.connect(lambda:(
			self.repaintChat(),
		...
		self.start.addWidget(QLabel("Tornado uses Tor's hidden services to send messages securely."))
		self.start.addWidget(QLabel("It's completely peer-to-peer, and no central server is involved."))
		self.start.addWidget(QLabel("Before continuing, make sure that the Tor Browser is running."))
		self.start.addWidget(QLabel("Once the browser is connected, hit the button below to begin."))
		...
		self.failed.addWidget(QLabel("It seems like the Tor Browser isn't running. Start it and try again."))
		...
		self.testfailed.addWidget(QLabel("Self-test failed. Ensure you are connected to the Internet and try again."))
		...
		self.connected.addWidget(QLabel("Connected successfully! You are now ready to start chatting. Keep in mind:"))
		self.connected.addWidget(QLabel("- You must keep Tornado running to receive messages. Minimizing the window is fine."))
		self.connected.addWidget(QLabel("- Your identity, contacts, and messages will be stored locally in your home directory."))
		self.connected.addWidget(QLabel("- Do not start more than one instance of Tornado because bad things will happen."))
		self.connected.addWidget(QLabel("- Nothing is completely secure. Make sure to use common sense and exercise caution."))
		self.connectedButton = QPushButton("Got it")
		self.connectedButton.clicked.connect(lambda:(
		...
		self.contacts.horizontalHeader().setSectionResizeMode(0,QHeaderView.ResizeMode.Stretch)
		...
		self.viewer.horizontalHeader().setSectionResizeMode(0,QHeaderView.ResizeMode.Stretch)
	...
	def connect(self):
		...
		try:
			...
			if not exists(expanduser("~/.tornado/key")):
				svc = controller.create_ephemeral_hidden_service({80:port},await_publication=True)
				...
				svc = controller.create_ephemeral_hidden_service({80:port},
					key_type=keyType,
			...
			try:
				res = get(f"http://{myaddr}.onion/alive",proxies={"http":"socks5h://127.0.0.1:9150"})
	...
	def sync(self):
		for i in range(len(sending.keys())-1,-1,-1):
			...
			for j in range(len(sending[addr])):
				...
				try:
					res = post(f"http://{addr}.onion/message",proxies={"http":"socks5h://127.0.0.1:9150"},data=message)
					if res.status_code!=204:
						raise Exception()
					sending[addr][j] = None
				except: pass
